...
| D13 | 节奏 | Phase 0 风格锁定（3–4 周）→ Phase 1 天龙垂直切片 → Phase 2 天龙全量 → 之后每书界一个周期；资产人工工时是主要成本（标准档约 400+ h/书界），必须以"资产分级 S/A/B/C"控制范围 | §3.3、§8 |
...
格式：`<前缀>_<主体ID或书界>_<描述>[__<变体>]`，全小写、`__`（双下划线）分隔变体，禁止 `@`、空格与中文（避免 URL/文件系统问题）。

| 前缀 | 类型 | 例 |
|---|---|---|
| `ref_` | 设定卡/参考图（不进运行时） | `ref_npc_xiaofeng__ch01_sheet` |
| `art_` | 概念原画 | `art_rg_01_dali_01` |
| `por_` | 立绘 | `por_npc_guojing__ch03_base`、`por_npc_guojing__ch03_e_angry` |
...
3. **题跋与印章**：CG 可留出右上/左上竖排题跋区；**文字一律后期用字体排版**（§2.8 禁止项 7），印章用程序化生成的朱文/白文印（字体 + 印泥纹理）。
...
| 精灵 3D 模型（S3） | 统一 **5.5 头身**"游戏比例"：头 ×1.25、手 ×1.15、兵器 ×1.2，其余按设定卡 | 用 Blender 形态键（shape key）从设定比例派生，保证 194 px 高度下面部与兵器可读；立绘与精灵之间不要求比例一致，只要求**识别锚**一致（§7.2） |
...
| 地标建筑 | 8 | ≈ 115 | S/A | 少林寺、天龙寺、灵鹫宫、桃花岛、襄阳城、古墓、光明顶、黑木崖、侠客岛石室、紫禁城…… |
| 道具物件 `prp_` | 30 | ≈ 450 | B/C | 大量复用 |
| 图标 `ico_` | ≈ 150 | ≈ 2,000（独立绘制 ≈ 1,000，其余模板变体） | A/B/C | 见下表拆分 |
| 特效贴图 `vfx_` | 天级武学签名特效 3–8 | 通用 ≈ 80 纹理/序列 + 签名 ≈ 55 | A | 天级武学（§13 共 55 部）每部一个签名特效 |
...
```mermaid
gantt
  title 素材生产节奏（示意：以 2026-10 起算，每周 12–15 小时业余投入）
  dateFormat YYYY-MM-DD
  axisFormat %Y-%m
  section Phase 0 风格锁定
  美术圣经定稿·风格 LoRA·3 个测试角色走通全链路 :p0, 2026-10-05, 28d
  section Phase 1 天龙垂直切片
  切片素材（2 区域·1 幕主线·3 队友·1 Boss） :p1, after p0, 70d
  section Phase 2 天龙全量
  天龙其余区域·角色·CG·视频 :p2, after p1, 168d
  section Phase 3+ 后续书界
  射雕（首个复用周期） :p3, after p2, 210d
```

| 阶段 | 目标 | 交付物 | 通过标准 |
|---|---|---|---|
| **Phase 0 风格锁定**（3–4 周） | 证明"风格可控 + 管线可跑通" | 美术圣经 v1；`style_gongbi_fine`/`style_shuimo_scene`/`style_gongbi_game` 三个风格 LoRA v1；主角（北宋装）、段誉、一个杂兵三者走完"设定卡 → 立绘 → 3D → 8 向精灵 → 进引擎"；1 张 CG；1 组图标模板；Blender 渲染脚本 v1 | 在真机（中端安卓 + iPhone）上精灵、立绘、图标观感一致；同一角色 10 张不同构图的识别一致性人工评分 ≥ 4/5 |
| **Phase 1 天龙垂直切片**（≈ 10 周） | 一个完整可玩片段的全部素材 | 区域：大理 + 无量山（`rg_01_dali`、`rg_01_wuliang`）；角色：主角、书灵、段誉、钟灵、木婉清、南海鳄神（Boss）、无量剑派弟子/神农帮帮众（杂兵，模块化）；动物：蛇、闪电貂；共享动作库 v1（通用 + 剑 + 刀 + 指 + 掌）；宋式建筑套件 v1 + 3 地标；地形材质 12；图标 ≈ 120；签名特效 3（一阳指、六脉神剑、凌波微步残影）；BGM 5；音效 ≈ 120；视频 1（`vid_ch01_intro`） | 切片可连续游玩 30–60 分钟无占位图；管线每类资产都有 ≥ 1 次"从登记到入 manifest"的全自动记录 |
| **Phase 2 天龙全量**（≈ 24 周） | 首个完整书界 | 其余区域（雁门关、少林、燕子坞、缥缈峰、辽国南京……）、全部队友/Boss、≈ 20 CG、开场视频、天书现世、书眠（天龙→射雕） | 天龙书界可通关 |
| **Phase 3+** | 每书界一个周期 | 逐部复用：射雕/神雕共享宋金蒙古套件与大量人物（年龄变体）；倚天复用神雕部分人物；清代 7 部共享清式套件 | 每部周期目标逐步缩短（见 §8.4） |
...
flowchart LR
  ...
  M2 --> M3[游戏比例形态键<br/>头1.25 手1.15 兵器1.2]
...
| 精英（不镜像、单兵器类） | `loco5m`(16×5) + `battle4`(37+18=55 ×4) | 80 + 220 = 300 | ≈ 9 MB |
...
- 最终上限由 `tech/03` 裁定；本文脚本通过 `sprite-spec.json` 的 `ppm`、`fps_step`、`dirs` 可一键重渲适配。
...
合计约 **47 条、约 27 分钟**成片；AI I2V 镜头约 150–250 个（每个 5–10 s）。**游戏内大部分剧情演出用引擎实时演出（精灵 + 立绘 + Ink 对话），视频只用于上表时刻。**
...
│  ├─ ch01/{characters,portrait,sprite,cg,icon,terrain,building,audio,video}.yaml
│  └─ ...
└─ tools/
...
| 视频 I2V（可灵 3.0 / Veo 3.1 Fast，含 3–5 倍重试） | Veo Fast $0.10–0.15/s；可灵 6–12 积分/s | 1 条 ≈ $40 | 3 条 ≈ $80–150 | 47 条 ≈ **$1,000–2,000** |
...
| **合计** | | **≈ $190–210** | **≈ $170–260** | **≈ $2,700–3,700**（多年摊销） |
